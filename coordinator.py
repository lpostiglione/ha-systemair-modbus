...
class SystemairCoordinator(DataUpdateCoordinator[dict[int, int]]):
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            rr1 = await self.client.read_holding_registers(address=100, count=12, slave=self.unit)
            # 201..220 (heater/cooler types, signals, temp level/sp, temps in1..in5, preheater type)
            rr2 = await self.client.read_holding_registers(address=200, count=20, slave=self.unit)
            # 351 (rotor state)
            rr3 = await self.client.read_holding_registers(address=350, count=1, slave=self.unit)
            # 301 (damper pwm)
            rr_damper = await self.client.read_holding_registers(address=300, count=2, slave=self.unit)
            # 381..383 (RH value + data valid)
            rr_rh = await self.client.read_holding_registers(address=380, count=4, slave=self.unit)
            # 651..655 (defrosting/RH presence)
            rr_defr = await self.client.read_holding_registers(address=650, count=6, slave=self.unit)
            # 601..602 (filter info)
            ...
            data[REG_SYSTEM_BOOT_PROG_V_LOW] = get(rr_sys, 500, REG_SYSTEM_BOOT_PROG_V_LOW)
