...
class SystemairCoordinator(DataUpdateCoordinator[dict[int, int]]):
    ...
    async def _async_update_data(self):
        ...
        async with self._io_lock:
            ...
            try:
                ...
                # 101..114 (includes fan speed level, SF/EF PWM/RPM, and manual stop allowance)
                rr1 = await self.client.read_holding_registers(address=100, count=14, device_id=self.unit)
                # 201..220 (heater/cooler types, signals, temp level/sp, temps in1..in5, preheater type)
                rr2 = await self.client.read_holding_registers(address=200, count=20, device_id=self.unit)
                # 351 (rotor state)
                rr3 = await self.client.read_holding_registers(address=350, count=1, device_id=self.unit)
                # 301 (damper pwm)
                rr_damper = await self.client.read_holding_registers(address=300, count=2, device_id=self.unit)
                # 381..383 (RH value + data valid)
                rr_rh = await self.client.read_holding_registers(address=380, count=4, device_id=self.unit)
                # 651..655 (defrosting/RH presence)
                rr_defr = await self.client.read_holding_registers(address=650, count=6, device_id=self.unit)
                # 601..602 (filter info)
                rr4 = await self.client.read_holding_registers(address=600, count=2, device_id=self.unit)
                # 501..508 (system parameters)
                rr_sys = await self.client.read_holding_registers(address=500, count=8, device_id=self.unit)
                ...
                data[REG_FAN_ALLOW_MANUAL_FAN_STOP] = get(rr1, 100, REG_FAN_ALLOW_MANUAL_FAN_STOP)
    ...
    async def async_write_register(self, address: int, value: int):
        ...
        async with self._io_lock:
            ...
            await self.client.write_register(address=(address - 1), value=value, device_id=self.unit)
